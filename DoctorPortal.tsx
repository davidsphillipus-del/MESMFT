...
import { Printer, Calendar, Search, Users, MessageSquare, Brain } from "lucide-react";
...
function TopProfile({ doctor }) {
  return (
    <Card className="rounded-2xl">
      <CardContent>
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-300 to-blue-500 text-white flex items-center justify-center font-semibold text-xl shadow-inner">{doctor.name.split(" ").map(n => n[0]).slice(0,2).join("")}</div>
          <div>
            <div className="font-semibold text-lg">{doctor.name}</div>
            <div className="text-sm text-gray-600">{doctor.id}</div>
            <div className="text-sm text-gray-500 mt-1">{doctor.specialty} • {doctor.years} yrs</div>
            <div className="text-sm text-gray-500">Contact: {doctor.contact}</div>
          </div>
        </div>
      </CardContent>
    </Card>
...
// Explore patients (doctor view of patient list)
...
// Main Doctors portal — layout mirrors PatientPortal but tailored for doctor
export default function DoctorsPortal() {
  ...
  function addObservation(epId, text) {
    setEpisodes(prev => prev.map(ep => ep.id === epId ? { ...ep, observations: [...ep.observations, { who: MOCK_DOCTOR.name, time: new Date().toISOString(), note: text }] } : ep));
  }

  function addDiagnosis(epId, text) {
    setEpisodes(prev => prev.map(ep => ep.id === epId ? { ...ep, diagnoses: [...ep.diagnoses, { id: `D-${Date.now()}`, doctor: MOCK_DOCTOR.name, text, createdAt: new Date().toISOString() }] } : ep));
  }

  function addPrescription(epId, text) {
    setEpisodes(prev => prev.map(ep => ep.id === epId ? { ...ep, prescriptions: [...ep.prescriptions, { text, prescriber: MOCK_DOCTOR.name, createdAt: new Date().toISOString(), dispensed: false }] } : ep));
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white pb-12">
      <header className="bg-white shadow sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="text-sm text-gray-600">Signed in as <strong>{MOCK_DOCTOR.name}</strong></div>
        ...
        <section className="lg:col-span-3 space-y-6">
          {tab === 'home' && (
            <div className="space-y-6">
              <SectionCard title={`Welcome back, ${MOCK_DOCTOR.name.split(" ")[1] || MOCK_DOCTOR.name.split(" ")[0]}`} subtitle={`ID: ${MOCK_DOCTOR.id}`} icon={<Users className="w-5 h-5" />}>
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <SectionCard title="Diagnosis Bot" subtitle="Clinician assistant" icon={<Brain className="w-5 h-5" />}>
                      <div className="p-4 text-sm text-gray-700">Run clinician smart prompts to get prioritized differential diagnoses and suggested tests. Outputs must be reviewed by clinicians.</div>
                      <div className="mt-3"><a href="/diagnosis-chat" className="inline-block bg-red-600 text-white px-4 py-2 rounded-md">Open Diagnosis Chat</a></div>
                    </SectionCard>
                  </div>
                </div>
              </SectionCard>
